...
export default function App() {
  ...
  // Synchronize admin authentication state with the sound manager
  useEffect(() => {
    orderSoundManager.setAdminAuthenticated(isAdminAuthenticated);
  }, [isAdminAuthenticated]);

  // Real-time sound notification for new orders — EXCLUSIVELY for authenticated Admin
  const prevAdminOrdersCountRef = useRef(ordersHistory.length);
  const isInitialAdminOrdersMount = useRef(true);

  useEffect(() => {
    if (!isAdminAuthenticated) {
      isInitialAdminOrdersMount.current = true;
      orderSoundManager.stopLoop();
      return;
    }

    if (isInitialAdminOrdersMount.current) {
      isInitialAdminOrdersMount.current = false;
      prevAdminOrdersCountRef.current = ordersHistory.length;
      return;
    }

    if (ordersHistory.length > prevAdminOrdersCountRef.current) {
      const newestOrder = ordersHistory[0];
      if (newestOrder && (newestOrder.status === 'recebido' || !newestOrder.status)) {
        if (orderSoundManager.isEnabled()) {
          orderSoundManager.startOrderRingtoneLoop();
        }
      }
    }
    prevAdminOrdersCountRef.current = ordersHistory.length;
  }, [ordersHistory, isAdminAuthenticated]);
  ...
  const handleOrderCompleted = (summaryData: any, whatsappUrl: string, rawMessage: string) => {
    ...
    // Play iFood telephone style ring for incoming order (ONLY if currently authenticated as admin)
    if (isAdminAuthenticated) {
      try {
        orderSoundManager.startOrderRingtoneLoop();
      } catch {}
  ...
  const handleDeleteOrder = (orderId: string) => {
    setOrdersHistory((prev) => prev.filter((o) => o.orderId !== orderId && o.id !== orderId));
    dbDeleteOrder(orderId).catch(console.error);
  };
