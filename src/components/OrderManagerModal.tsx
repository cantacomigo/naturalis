...
export const OrderManagerModal: React.FC<OrderManagerModalProps> = ({
  ...
  // Sound notification state & auto-ring on incoming orders
  const [isSoundEnabled, setIsSoundEnabled] = useState(() => orderSoundManager.isEnabled());
  ...
  // Monitor incoming orders: trigger iFood telephone ring when a new received order arrives
  useEffect(() => {
    ...
    if (orders.length > prevOrdersCountRef.current) {
      ...
      if (newestOrder && (newestOrder.status === 'recebido' || !newestOrder.status)) {
        if (isSoundEnabled) {
          orderSoundManager.startOrderRingtoneLoop();
          setIsRinging(true);
          setRingingOrderId(newestOrder.orderId);
          showToast(`🔔 Novo Pedido #${newestOrder.orderId} Recebido!`);
        }
      }
    }
    prevOrdersCountRef.current = orders.length;
  ...
  // Clean up ringing loop when modal unmounts or closes
  useEffect(() => {
    return () => {
      orderSoundManager.stopLoop();
    };
  }, []);
  ...
  const handleStopRingtone = () => {
    ...
    showToast('Toque silenciado.');
  };

  const handleToggleSound = () => {
    const next = !isSoundEnabled;
    setIsSoundEnabled(next);
    orderSoundManager.setEnabled(next);
    if (!next) {
      handleStopRingtone();
      showToast('Som de novos pedidos desativado.');
    } else {
      orderSoundManager.playIFoodOrderAlert();
      showToast('🔊 Som ativado! Toque estilo iFood pronto.');
    }
  };

  const handleTestSound = () => {
    orderSoundManager.playIFoodOrderAlert();
    showToast('🔊 Tocando alerta estilo iFood / Telefone...');
  };
      ...
      >
        ...
        <div className="p-4 sm:p-5 bg-white text-stone-900 flex items-center justify-between border-b border-stone-100 shrink-0">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            {/* Sound Toggle (iFood order phone ring) */}
            <button
              type="button"
              ...
              title={isSoundEnabled ? 'Som de novos pedidos ativado (Estilo iFood). Clique para silenciar.' : 'Som desativado. Clique para ativar toque de novos pedidos.'}
              aria-label="Alternar som de novos pedidos"
            >
              {isSoundEnabled ? <Volume2 className="w-4 h-4 text-emerald-600" /> : <VolumeX className="w-4 h-4 text-stone-400" />}
              <span className="hidden md:inline">{isSoundEnabled ? 'Som iFood ON' : 'Som Mudo'}</span>
            </button>
            ...
            <button
              ...
              className="p-2 text-stone-600 hover:text-stone-900 hover:bg-stone-100 border border-stone-200/80 rounded-xl transition-all cursor-pointer text-xs font-semibold flex items-center gap-1"
              title="Testar toque de telefone / iFood para novos pedidos"
              aria-label="Testar som"
            >
              <Bell className="w-4 h-4 text-amber-500" />
              <span className="hidden lg:inline">Testar Toque</span>
            </button>
        ...
        {/* Active Incoming Order Phone Ring Banner */}
        {isRinging && (
          <div className="bg-gradient-to-r from-rose-600 via-amber-600 to-rose-600 text-white px-4 sm:px-6 py-3 flex items-center justify-between gap-3 shadow-lg animate-pulse shrink-0">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-white text-rose-600 rounded-2xl shadow-sm animate-bounce">
              ...
              <div>
                ...
                <p className="text-xs text-rose-100 font-medium">
                  {ringingOrderId ? `Pedido #${ringingOrderId} aguardando conferência.` : 'Verifique os novos pedidos recebidos na lista.'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3.5 py-1.5 bg-white hover:bg-rose-50 text-rose-700 font-black text-xs rounded-xl shadow-sm cursor-pointer transition-all active:scale-95"
              >
                Silenciar Toque
              </button>
            </div>
