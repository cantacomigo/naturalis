...
export const HeroBanner: React.FC<HeroBannerProps> = ({
  ...
  return (
    <section className="pt-2 pb-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      {/* Live Social Proof / Store Activity Ticker (ALWAYS VISIBLE) */}
      ...
      {/* Admin indicator when Bento Showcase is hidden */}
      {!isBentoVisible && isAdminAuthenticated && onToggleHeroBanner && (
        <div className="mb-4 p-3 bg-purple-50/90 border border-purple-200 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-purple-950 shadow-2xs">
          <div className="flex items-center gap-2.5">
            <span className="p-1.5 bg-purple-100 rounded-xl text-sm">👁️‍🗨️</span>
            <span><strong>Modo Lojista:</strong> A seção de Carrossel e Destaques abaixo está <strong>oculta</strong> para os clientes.</span>
          </div>
          <button
            type="button"
          ...
          >
            Tornar Destaques Visíveis
          </button>
        </div>
      )}

      {/* Main Bento Grid (Carousel + Actions + Mini Spotlight Cards) */}
      {isBentoVisible && currentSlide && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 animate-in fade-in duration-300">
