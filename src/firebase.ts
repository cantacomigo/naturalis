...
// Orders History Listener - Realtime across all browsers/devices
...
export async function dbSaveOrder(order: OrderRecord) {
  try {
    const docId = order.orderId || order.id || `ord-${Date.now()}`;
    ...
    console.log('✅ Pedido salvo com sucesso no Firebase Firestore:', docId);
    return sanitizedOrder;
  } catch (err) {
    console.error('❌ Erro crítico ao salvar pedido no Firebase Firestore:', err);
    throw err;
  }
}
...
export async function dbDeleteOrder(orderId: string) {
  try {
    ...
    await deleteDoc(docRef);
    console.log('🗑️ Pedido removido do Firebase:', orderId);
  } catch (err) {
    console.error('❌ Erro ao deletar pedido do Firebase:', err);
    throw err;
  }
}
...
export async function dbAddStockMovement(movement: StockMovement) {
  try {
    ...
    await setDoc(docRef, sanitizeForFirestore(movement), { merge: true });
  } catch (err) {
